...
// Suggestions for station/city search
const STATIONS = [
...
// ---------- Destination Search Modal (hotel-style) ----------
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            {list.map((s) => (
              ...
              >
                ...
                <View>
                  <Text style={{ color: TEXT, fontSize: 16, fontWeight: '600', fontFamily: 'Raleway_400Regular' }}>
                    {s.name} <Text style={{ color: SUBTLE, fontFamily: 'Raleway_400Regular' }}>({s.code})</Text>
                  </Text>
                  <Text style={{ color: SUBTLE, fontFamily: 'Raleway_400Regular' }}>{s.country}</Text>
                </View>
              </TouchableOpacity>
            ))}
          </ScrollView>
...
// ---------- ResultsContent (adapted to trains) ----------
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0E141C' }}>
      <View style={{ height: 48, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12 }}>
        ...
        <Text style={{ color: TEXT, fontSize: 16, fontWeight: '700', flex: 1, textAlign: 'center', fontFamily: 'Raleway_400Regular' }}>{titleText}</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        {offers.map((o) => (
          ...
          >
            ...
            <View style={styles.resRoute}>
              <Text style={styles.resApt}>{o.stationFrom}</Text>
              <View style={styles.resDivider} />
              <Text style={styles.resApt}>{o.stationTo}</Text>
            </View>
        ...
        <TouchableOpacity style={styles.priceChartBtn} onPress={onOpenPriceChart}>
          ...
          <Text style={{ color: TEXT, fontWeight: '700', fontFamily: 'Raleway_400Regular' }}>Price chart</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
export default function TrainSearch() {
  ...
  // Top tabs (Train active)
const TopTabs = ({ active = 'Train' }) => {
  const tabs = [
  ...
  // mock cards
  const lastMinute = [
    { price: 79, from: 'Paris', to: 'Brussels', time: '1h 22m · Direct', day: '19 Dec, Thu', range: '12:10 – 13:32' },
    { price: 95, from: 'Amsterdam', to: 'Berlin', time: '6h 10m · 1 transfer', day: '19 Dec, Thu', range: '09:05 – 15:15' },
  ];
  const explore = [
    { city: 'Paris', fromPrice: 39, img: 'https://images.unsplash.com/photo-1508057198894-247b23fe5ade?w=600&q=60' },
    { city: 'Amsterdam', fromPrice: 29, img: 'https://images.unsplash.com/photo-1505764706515-aa95265c5abc?w=600&q=60' },
    { city: 'Prague', fromPrice: 41, img: 'https://images.unsplash.com/photo-1505761671935-60b3a7427bad?w=600&q=60' },
  ];

  // results (mock)
  const offers = [
    { id: '1', operator: 'TGV INOUI', depart: '08:12', arrive: '10:38', stationFrom: 'PAR', stationTo: 'AMS', duration: '2h 26m', transfers: 'Direct', price: 85 },
    { id: '2', operator: 'Thalys',    depart: '09:40', arrive: '12:06', stationFrom: 'PAR', stationTo: 'AMS', duration: '2h 26m', transfers: 'Direct', price: 92 },
    { id: '3', operator: 'DB + NS',   depart: '07:15', arrive: '12:45', stationFrom: 'PAR', stationTo: 'AMS', duration: '5h 30m', transfers: '1 transfer', price: 69 },
    { id: '4', operator: 'ICE + IC',  depart: '14:05', arrive: '20:20', stationFrom: 'PAR', stationTo: 'AMS', duration: '6h 15m', transfers: '2 transfers', price: 58 },
  ];

  // helpers
  const openResults = () => {
    const pick = [from, to].filter(Boolean);
    const next = [...new Set([`${from} → ${to}`, ...recent])].slice(0, 10);
    if (pick.length === 2) setRecent(next);
  ...
  return (
    <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.labeledInput}>
            ...
            <TouchableOpacity style={styles.inputRow} activeOpacity={0.9} onPress={() => openDest('from')}>
              <Text style={styles.inputText}>{from || 'City / Station'}</Text>
          ...
          <View style={[styles.duoRow, { marginTop: 12 }]}>
            ...
            {/* No return (trip-type pills removed as requested) */}
          ...
          {lastMinute.map((c, idx) => (
            <View key={idx} style={styles.ticketCard}>
              ...
              <View style={styles.ticketRow}>
                <MaterialCommunityIcons name="train" size={14} color={SUBTLE} />
      ...
      {/* Filters Modal (flight labels -> train labels) */}
          ...
          <View style={styles.filterSheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
              <View style={styles.group}>
                <Text style={styles.groupTitle}>Train information</Text>
                ...
                <TouchableOpacity style={styles.selectorRow} activeOpacity={0.8}>
                  <Text style={styles.selectorText}>Select train types / classes</Text>
                  <Ionicons name="chevron-forward" size={16} color={SUBTLE} />
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.group}>
                ...
                {[
                  { key: 'direct', label: 'Direct trains' },
                  ...
                  >
                    ...
                    <Text style={styles.checkPrice}>$59</Text>
                  </TouchableOpacity>
                ))}
              </View>
              ...
              <View style={styles.group}>
                ...
                <View style={styles.sliderHeader}>
                  <Text style={styles.sliderPillPlain}>1hr</Text>
                  <Text style={styles.sliderPillPlain}>24hrs</Text>
                </View>
                <RangeSlider
                  min={1}
                  max={24}
                  step={1}
                  values={[Math.max(1, filters.stops[0] || 1), Math.max(1, filters.stops[1] || 24)]}
              ...
              <View style={styles.group}>
                ...
                <RangeSlider min={0} max={10000} step={50} values={[filters.price[0], filters.price[1]]} onChange={(vals) => setFilters((f) => ({ ...f, price: vals }))} />
              </View>

              <View style={styles.group}>
                {[
                  { key: 'breg', label: 'No seat reservation required' },
              ...
              {selectedOffer && (
                <View style={{ backgroundColor: CARD_BG, borderRadius: 14, borderWidth: 1, borderColor: BORDER, padding: 14 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <MaterialCommunityIcons name="train" size={16} color={SUBTLE} />
                  ...
                  <Text style={{ color: TEXT, fontWeight: '700', marginBottom: 6, fontFamily: 'Raleway_400Regular' }}>2nd Class</Text>
                  <View style={{ gap: 4 }}>
                    <Text style={{ color: SUBTLE, fontSize: 12, fontFamily: 'Raleway_400Regular' }}>• Seat reservation optional</Text>
          ...
          <View style={{ backgroundColor: '#1C2030', borderTopLeftRadius: 16, borderTopRightRadius: 16, paddingTop: 12, paddingBottom: 20, maxHeight: Math.min(height * 0.5, 380) }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 8 }}>
              {Array.from({ length: 16 }).map((_, i) => {
                const price = 20 + Math.round(Math.abs(Math.sin(i * 1.1)) * 80);
                const barH = Math.max(24, Math.min(120, Math.round((price / 100) * 120)));
                return (
...
const styles = StyleSheet.create({
  ...
  ticketLabel: { color: SUBTLE, fontSize: 12 },
  ticketSub: { color: TEXT, fontSize: 12, marginTop: 2 },
  ...
  paxBtnText: { color: '#fff', fontSize: 20, lineHeight: 20 },
  paxCount: { color: TEXT, fontSize: 16, marginHorizontal: 12 },
  ...
  checkPrice: { color: SUBTLE, fontSize: 12 },
  ...
  resChipText: { color: TEXT, fontSize: 12 },
  ...
  resTime: { color: TEXT, fontSize: 16, fontWeight: '700', width: 56, textAlign: 'center' },
  resAirline: { color: TEXT, fontSize: 13, fontWeight: '700' },
  resMeta: { color: SUBTLE, fontSize: 12, marginTop: 2 },
  resRoute: { flexDirection: 'row', alignItems: 'center', marginTop: 2, gap: 8, paddingHorizontal: 6 },
  resApt: { color: SUBTLE, fontSize: 12 },
